...
def index():
    ...
    no10=pd.crosstab(
        ...
        margins = True,
        margins_name = 'Jumlah'
        ).sort_values(by='Jumlah', ascending = False).drop('Jumlah', axis=1).drop('Jumlah', axis=0)
    no10.head().plot.bar(figsize = (12,12),stacked=True, fontsize = 12)
    plt.savefig('stack_bar.png',bbox_inches="tight")
